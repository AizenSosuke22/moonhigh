...
<div id="app" class="mr-auto sm:w-[calc(100%-40px)] md:w-[calc(100%-250px)] bg-[white] h-fit min-h-screen px-[50px] pt-[30px]">
    ...
    <form action="{{ url('/sales/add') }}" method="POST" class="w-full h-fit block rounded-md mt-[60px] px-[40px] pb-[60px]">
        @csrf
        ...
        @endforeach
        <div class="my-4">
          <label for="" class="text-[14px]">نسبة التخفيض</label>
          <input type="number" required min="0" max="100" name="percent" value="" class="w-full border h-fit text-[15px] px-4 py-2 rounded-md bg-[#f0f0f041] mt-2 outline-none">
        </div>
        <div class="my-4">
            <label for="" class="text-[14px]">اختر على الاقل صنفا</label>
            <select name="service[]" required multiple class="w-full border h-fit text-[15px] px-4 py-2 rounded-md bg-[#f0f0f041] mt-2 outline-none">
